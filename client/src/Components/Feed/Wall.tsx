...
function orderByUCB(hashMap,ucb) {
  ...
  return values.sort(UCBcompare)
  } else {
    let values = Object.values(hashMap)
    return values.sort(Reactscompare)
  }
  return null
}
...
const mapStateToProps = state => {
  ...
  return { merchant: state.merchant, tags: ordered, live: state.live, ucb: state.ucb }
}
...
const ConnectedWall = (props) => {
  ...
  const { data, error, loading, fetchMore } = useQuery(GET_TAGS, {variables: {id: props.merchant.id, ucb: props.ucb}, pollInterval: props.live?2500:0})
  ...
  return (
    <Fragment>
      {((props.live && data.merchantTags!==null)? data.merchantTags:props.tags).map((tag: any) => 
        <Tag tagID={tag.id} total={tag.reacts} content={tag.content} key={tag.id} color={colors[Math.floor(Math.random() * colors.length)]}/>
      )}
      { moreToGet && 
        <div onClick={() => fetchMore({ variables: {id: props.merchant.id, ucb: props.ucb}, 
